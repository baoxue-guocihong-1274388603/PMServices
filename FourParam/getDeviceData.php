<?php
	...
	if ($OperationType === "AddDevice") {
		...
		$sql = "INSERT INTO dtm_deviceinfo_multiparam_table VALUES('".$DeviceID."','".$MonitorPoint."')";
		...
		$sql = "DELETE FROM dtm_deviceinfo_multiparam_table WHERE device_id = '".$DeviceID."'";
		$result = mysqli_query($link,$sql);
	...
	} else if( $OperationType === "GetDeviceInfo") {
		//什么都不做
	}
	
	$DeviceData = array();
	$sql = "SELECT * FROM dtm_deviceinfo_multiparam_table ORDER BY device_id ASC";
	$result = mysqli_query($link,$sql);
