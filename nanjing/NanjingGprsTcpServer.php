	...
	class Server {
		...
		public function __construct() {
			...
			$this->serv->set(array(
				...
				//'tcp_keepidle' => 15,//连接在15秒内没有数据请求,将开始对此连接进行探测
				//'tcp_keepcount' => 3,//探测的次数,超过次数后将close此连接
				//'tcp_keepinterval' => 3,//探测的间隔时间,单位秒
				'log_file' => '/DataDisk/log/nanjing/swoole_gprs.log',
		...
		public function onReceive(swoole_server $serv, $fd, $from_id, $data) {
			...
			for ($i = 0; $i < strlen($str); $i += 8)  {
				...
				$sql = array('sql' => 'INSERT INTO dtm_pminfo_nanjing_table VALUES(?, ?, ?, ?, ?)', 'param' => array($DeviceID, $PmValue, $TriggerTime, $PmQuality, $BgColor)); 
		...
		public function onTask($serv, $task_id, $from_id, $data) { 
			...
			//判断设备有没有注册
			$res = $this->pdo->query("SELECT * FROM dtm_deviceinfo_nanjing_table WHERE device_id = '".$sql['param'][0]."'");
			if ($res->rowCount() == 1) {
				try { 				
