...
foreach ($DeviceIDList as $Value) {
	...
	SELECT
		...
		t2.triggertime
	FROM
		`dtm_pminfo_debug_table` AS t2
	INNER JOIN dtm_deviceinfo_debug_table AS t1 ON t1.device_id = t2.device_id
	WHERE
		t2.device_id = ' . $Value . '
	ORDER BY
		triggertime DESC
...
FROM
	`dtm_pminfo_debug_table` AS t2
INNER JOIN dtm_deviceinfo_debug_table AS t1 
...
LIMIT 1';
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $TableData[] = array("AddressName" => $row[0], "PmValue" => $row[1], "Temperature" => $row[2], "Humidity" => $row[3], "PmQuality" => $row[4], "BgColor" => $row[5]);
    }
    mysqli_free_result($result);
}
