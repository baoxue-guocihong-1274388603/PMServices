<?php
	...
	$sql = "SELECT device_id,`pmvalue`,triggertime FROM dtm_pminfo_debug_table WHERE device_id in ".$DeviceID." ORDER BY triggertime DESC LIMIT 0,100";
	$result = mysqli_query($link,$sql);
	while($row = mysqli_fetch_array($result)){
		$res = mysqli_query($link,"SELECT monitor_point FROM dtm_deviceinfo_debug_table WHERE device_id = '".$row[0]."'");
	...
	$sql = "SELECT * FROM (SELECT * FROM dtm_pminfo_debug_table WHERE device_id in ".$DeviceID." ORDER BY triggertime desc) pcc GROUP BY device_id";
	$result = mysqli_query($link,$sql);
	while($row = mysqli_fetch_array($result)){
		$res = mysqli_query($link,"SELECT monitor_point FROM dtm_deviceinfo_debug_table WHERE device_id = '".$row[0]."'");
		$r = mysqli_fetch_array($res);

		$TableData[] = array("AddressName"=>$r[0],"PmValue"=>$row[1],"Temperature"=>$row[2],"Humidity"=>$row[3],"PmQuality"=>$row[5],"BgColor"=>$row[6]);
	}
