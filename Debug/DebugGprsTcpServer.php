<?php
	...
	class Server {
		...
		public function __construct() {
			$this->serv = new swoole_server("139.196.12.213",9090,SWOOLE_PROCESS,SWOOLE_TCP);
			...
			$this->serv->set(array(
				...
				'log_file' => '/DataDisk/log/debug/swoole_gprs.log',
		...
		public function onReceive(swoole_server $serv, $fd, $from_id, $data) {
			...
			for ($i = 0; $i < strlen($str); $i += 20)  {
				...
				$DeviceID = hexdec(substr($ReceiveData, 0, 8));
				$PmValue = hexdec(substr($ReceiveData, 8, 4));
				$Temperature = (hexdec(substr($ReceiveData, 12, 4)) - 500) / 10;//温度
				$Humidity = hexdec(substr($ReceiveData, 16, 4)) / 10;//湿度
				...
				echo "Temperature Value:" . $Temperature . PHP_EOL;
				...
				$sql = array('sql' => 'INSERT INTO dtm_pminfo_debug_table VALUES(?, ?, ?, ?, ?, ?, ?)', 'param' => array($DeviceID, $PmValue, $Temperature, $Humidity, $TriggerTime, $PmQuality, $BgColor)); 
		...
		public function onTask($serv, $task_id, $from_id, $data) { 
			...
			//判断设备有没有注册
			$res = $this->pdo->query("SELECT * FROM dtm_deviceinfo_debug_table WHERE device_id = '".$sql['param'][0]."'");
			if ($res->rowCount() == 1) {
				try { 				
