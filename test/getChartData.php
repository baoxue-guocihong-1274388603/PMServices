<?php
	...
	$sql = "SELECT device_id,`value`,triggertime FROM dtm_pminfo_test_table WHERE device_id in ".$DeviceID." ORDER BY triggertime DESC LIMIT 0,100";
	...
	$sql = "SELECT * FROM (SELECT * FROM dtm_pminfo_test_table WHERE device_id in ".$DeviceID." ORDER BY triggertime desc) pcc GROUP BY device_id";
	$result = mysqli_query($link,$sql);
	while($row = mysqli_fetch_array($result)){
		$res = mysqli_query($link,"SELECT monitor_point FROM dtm_deviceinfo_test_table WHERE device_id = '".$row[0]."'");
		$r = mysqli_fetch_array($res);
