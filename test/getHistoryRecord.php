<?php
	...
	if($Type === "Normal"){
		$sql = "SELECT * FROM dtm_pminfo_test_table WHERE device_id = '".$DeviceID."' AND triggertime BETWEEN '".$StartTime."' AND '".$EndTime."' ORDER BY triggertime DESC";

		$result = mysqli_query($link,$sql);
		if($result){
			if(mysqli_num_rows($result)){
				while($row = mysqli_fetch_array($result)){
					$res = mysqli_query($link,"SELECT monitor_point FROM dtm_deviceinfo_test_table WHERE device_id = '".$row[0]."'");
		...
		//循环获取每天的平均值,按时间升序排序
		for($i = 0; $i <= $Days; $i++){
			$sql = "SELECT FLOOR(AVG(value)),date(triggertime) FROM dtm_pminfo_test_table WHERE	date(triggertime) in (SELECT date(DATE_ADD('".$StartTime."',INTERVAL '".$i."' DAY))) AND device_id = '".$DeviceID."'";
		...
		$sql = "SELECT monitor_point FROM dtm_deviceinfo_test_table WHERE device_id = '".$DeviceID."'";
		$result = mysqli_query($link,$sql);
		if($result){
			if(mysqli_num_rows($result)){
				while($row = mysqli_fetch_array($result)){
