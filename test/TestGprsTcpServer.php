	...
	class Server {
		...
		public function __construct() {
			...
			$this->serv->set(array(
				...
				'log_file' => '/DataDisk/log/test/swoole_gprs.log',
		...
		public function onReceive(swoole_server $serv, $fd, $from_id, $data) {
			...
			for ($i = 0; $i < strlen($str); $i += 8)  {
				...
				$sql = array('sql' => 'INSERT INTO dtm_pminfo_test_table VALUES(?, ?, ?, ?, ?)', 'param' => array($DeviceID, $PmValue, $TriggerTime, $PmQuality, $BgColor)); 
		...
		public function onTask($serv, $task_id, $from_id, $data) { 
			...
			//判断设备有没有注册
			$res = $this->pdo->query("SELECT * FROM dtm_deviceinfo_test_table WHERE device_id = '".$sql['param'][0]."'");
			if ($res->rowCount() == 1) {
				try { 				
